...
DROP FUNCTION IF EXISTS tok2
;

CREATE FUNCTION tok2(user_input TEXT)
RETURNS TABLE(displaytext_v TEXT, class_v TEXT, id_in_class_v INTEGER, query_tokens_v TEXT, word_similarity_v REAL) AS $$
DECLARE
	input_lower TEXT := LOWER(user_input);
	tok1 TEXT := split_part(input_lower, ' ', 1);
...
BEGIN
	RETURN QUERY
	    SELECT
			...
			class AS class_v,
			id_in_class AS id_in_class_v,
			tok1 || ' | ' || tok2 AS query_tokens_v,
			CAST (
				100 *  COALESCE(word_similarity(tok1, searchtext_1) + word_similarity(tok2, searchtext_1), 0) + 
				10 * COALESCE(word_similarity(tok1, searchtext_2) + word_similarity(tok2, searchtext_2), 0) +
				1 * COALESCE(word_similarity(tok1, searchtext_3) + word_similarity(tok2, searchtext_3), 0)
			AS REAL) AS hit_index_v
		FROM 
			public.searchobjects
		WHERE (
